...
# Part 2 adds that there is a 4th group now rather than 3 groups total...
# this would probably be way cleaner if refactored into some divide + conquer-esque function
# but I'm just going to save my time and make another inner loop section...
...
for i in range(1, len(inp) - 1):
    ...
    print(i)
    for pick_i_packages in itertools.combinations(inp, i): # picking for first group
        ...
        if 3 * weight_to_balance != remaining_packages_weight: # 1/4 taken out, remaining weight must be 3/4 the total
            continue

        for j in range(1, len(remaining_packages) - 1):
            for pick_j_packages in itertools.combinations(remaining_packages, j): # picking for second group
                if sum(pick_j_packages) != weight_to_balance:
                    continue

                remaining_packages_two = remaining_packages[:]
                for package in pick_j_packages:
                    remaining_packages_two.remove(package)

                for k in range(1, len(remaining_packages_two) - 1):
                    for pick_k_packages in itertools.combinations(remaining_packages_two, k):
                        if sum(pick_k_packages) != weight_to_balance:
                            continue
